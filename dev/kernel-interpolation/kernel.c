...
static inline void
linear_interpolation_xy(VDOUBLE rel[2], VDOUBLE w[2][2])
{
	VDOUBLE del[2];
...
relative_position_grid(VDOUBLE x0, VDOUBLE x, VDOUBLE dx, VDOUBLE idx, VINDEX i0[1])
{
	VDOUBLE rel;
	VDOUBLE block_delta, block_rel, block_start, grid_delta;
...
void
interpolate_weights_xy(VDOUBLE x[2], VDOUBLE dx[2], VDOUBLE idx[2],
		VDOUBLE x0[2], VDOUBLE w[2][2], VINDEX i0[2])
{
	VDOUBLE delta_grid[2];
...
test_rel()
{
	...
	for(iv=0; iv<MAX_VEC; iv++)
	{
		...
		printf("i0[%d] = [%x %x]\n", iv, ((__v4si) i0[X])[iv], ((__v4si) i0[Y])[iv]);
		...
		assert(((__v4si) i0[X])[iv] == 1);
		assert(((__v4si) i0[Y])[iv] == 2);
	}
}
